...
@implementation ArduinoReferenceObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_arduinoreference);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any ArduinoReference patches in JS before a context is loaded... nice for 
...
{
	// create ArduinoReference global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_arduinoreference = [[ArduinoReferenceObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_arduinoreference];
	
	NSString *_arduinoreferenceNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_arduinoreferenceNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_arduinoreference valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.arduinoreference.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
